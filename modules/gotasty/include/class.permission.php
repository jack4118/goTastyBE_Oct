<?php
    ...
    **/
	...
	public function checkActionPermission($params) {
	    ...
	    if($module=="") {
		return array('status' => "error", 'code' => 1, 'statusMsg' => 'Module cannot be empty', 'data' => '');
	    }

            if($type=="") {
                return array('status' => "error", 'code' => 1, 'statusMsg' => 'Type cannot be empty', 'data' => '');
            }

            if($userId=="") {
                return array('status' => "error", 'code' => 1, 'statusMsg' => 'UserID cannot be empty', 'data' => '');
            }
		...
		$db->where("action", array("read", "add", "edit", "remove", "approve", "cancelled"), "IN");
		$arrPermission = $db->get("action_permission");

		$permission = array("read"=>false, "add"=>false, "edit"=>false, "remove"=>false, "approve"=>false);

		foreach($arrPermission as $key => $value) {
		    $permission[strtolower($value["action"])] = true;
		}

		return array('status' => "ok", 'code' => 0, 'statusMsg' => '', 'data' => $permission);
		...
		$permission = array();
		foreach($arrPermission as $key => $value) {
		    $db->where("id", $value["reference_id"] );
		    $referenceDetail = $db->getValue($value["reference_table"], $value["reference_column"] );
		    if($referenceDetail!="") $permission[] = $referenceDetail;
		}
		...
		return array('status' => "ok", 'code' => 0, 'statusMsg' => '', 'data' => $permission);
                ...
                return array('status' => "error", 'code' => 1, 'statusMsg' => 'Invalid Type', 'data' => '');
	    }
	}
