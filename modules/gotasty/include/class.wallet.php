<?php
    ...
    class Wallet
    {
        ...
        public function getTransactionHistory($params) {
            ...
            $result = $db->get("credit_transaction", $limit, "id, client_id, subject, from_id, to_id, SUM(amount) AS amount, remark, batch_id, creator_id, creator_type, created_at, portfolio_id, belong_id, type, coin_rate, data");
...
/*
                if($value['creator_type'] != 'System')
                {
                    $transaction['subject'] = $transactionSubject ? $transactionSubject : $value['subject'] ;   
                }
                else
                {
                    if($value['subject'] == 'Adjustment In')
                    {
                        $transaction['subject'] = 'System Adjustment In';
                    }
                }
*/
		// $transaction['subject'] = $transactionSubject ? $transactionSubject : $value['subject'] ;

                if ($transactionSubject) {
                    if($transactionSubject == 'System Adjustment Out'){
                        $newTransactionSubject = 'Adjust Out';
                    }else  if ($transactionSubject == 'System Adjustment In'){
                        $newTransactionSubject = 'Adjust In';
                    }else  if ($transactionSubject == 'Redeem Reward'){
                        $newTransactionSubject = 'Redeemed';
                    }else  if ($transactionSubject == 'Sales Reward'){
                        $newTransactionSubject = 'Purchase Reward';
                    }else  if ($transactionSubject == 'Sponsor Bonus'){
                        $newTransactionSubject = 'Referral Reward';
                    }else{
                        $newTransactionSubject = $transactionSubject;
                    }

                    $transaction['subject'] = $newTransactionSubject;
                } else {
                    if($value['subject'] == 'System Adjustment Out'){
                        $newTransactionSubject = 'Adjust Out';
                    }else  if ($value['subject'] == 'System Adjustment In'){
                        $newTransactionSubject = 'Adjust In';
                    }else  if ($value['subject'] == 'Redeem Reward'){
                        $newTransactionSubject = 'Redeemed';
                    }else  if ($value['subject'] == 'Sales Reward'){
                        $newTransactionSubject = 'Purchase Reward';
                    }else  if ($value['subject'] == 'Sponsor Bonus'){
                        $newTransactionSubject = 'Referral Reward';
                    }else{
                        $newTransactionSubject = $value['subject'];
                    }
                ...
                // if ($value['remark']) {
                //     $transaction['remark'] = $value['remark'];
                // } else {
                //     $transaction['remark'] = "-";
                // }
                ...
                if ($transactionSubject) {
                    if ($value['remark'] || $transactionSubject == 'Redeem Reward' || $transactionSubject == 'Sales Reward' || $transactionSubject == 'Sponsor Bonus') {
                        if ($transactionSubject == 'Redeem Reward'){
                            $db->where("id", $value['id']);
                            $getCTReferID = $db->getValue("credit_transaction", "reference_id");

                            $db->where("id", $getCTReferID);
                            $getSoNO = $db->getValue("sale_order", "so_no");

                            if(!$getCTReferID){
                                $getSoNO = '-';
                            }

                            $transaction['remark'] = $getSoNO;
                            ...
                            $db->where("id", $value['to_id']);
                            $getUsername = $db->getValue("client", "username");

                            $transaction['remark'] = $getUsername;
                    ...
                    if ($value['remark'] || $transactionSubject == 'Redeem Reward' || $transactionSubject == 'Sales Reward' || $transactionSubject == 'Sponsor Bonus') {
                        if ($value['subject'] == 'Redeem Reward'){
                            $db->where("id", $value['id']);
                            $getCTReferID = $db->getValue("credit_transaction", "reference_id");
        ...
        public function creditAdjustment($params,$type) {
            ...
            if(!empty($isMember))
            {
                $creditType = 'memberDef';
            }
            try
            {
                $data = Cash::insertTAccount($accountID, $receiverID, $creditType, $adjustmentAmount, $subject, $belongID, "", $db->now(), $batchID, $clientId, $remark);
            }
            catch(Exception $e)
            {
                return array('status' => "error", 'code' => 1, 'statusMsg' => $translations["E00216"][$language] /* Adjustment failed. */, 'data' => $data);
            }
